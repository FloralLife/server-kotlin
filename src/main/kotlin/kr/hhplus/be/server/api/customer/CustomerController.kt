...
class CustomerController(
  ...
  fun get(@PathVariable customerId: Long): CustomerResponse {
    return customerService.get(customerId).toResponse()
  }

  @GetMapping("/{customerId}/point")
  fun getPoint(@PathVariable customerId: Long): Int {
    return customerService.get(customerId).point
  }

  @PostMapping
  fun create(): CustomerResponse {
    return customerService.create().toResponse()
  }

  @PutMapping("/{customerId}/point")
  fun chargePoint(@PathVariable customerId: Long, @RequestBody request: PointChargeRequest): Customer {
    return customerService.chargePoint(customerId, request.amount)
  }
}
